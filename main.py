...
class ExpConfig():
    ...
    out_dim = 20   # the dimension of embedding
    d_model = 128
    window_length = 20
...
def main():
    ...
    for epoch in range(5):
        ...
        for inputs in tqdm(train_loader):
            # inputs = inputs.cuda()
            l0 = model(inputs)
            # loss1_list.append((l1 - 3 * series_loss).item())
            # loss1 = l0 - 3 * series_loss
            # loss2 = l1 + 3 * prior_loss
        ...
        t, th = bf_search(val_score, val_label[-len(val_score):], step_num=200)
        if(t[0] > f1):
            f1 = t[0]
    ...
    t, th = bf_search(test_score, test_label[-len(test_score):], step_num=200)
    print('*****************************************************')
